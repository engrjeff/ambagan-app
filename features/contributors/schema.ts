...
const contributorItemSchema = z.object({
  ...
  contributionAmount: z.number({ error: "Contribution amount is required." }).gt(0, { error: "Invalid amount." }),
  email: z
    .string()
    .optional()
...
export const contributorEditSchema = contributorItemSchema.extend(contributorIdSchema.shape);
...
export const contributorSchema = z.object({
  ...
  contributors: z
    ...
    .superRefine((items, ctx) => {
      const uniqueItemsCount = new Set(items.map((item) => item.name.toLowerCase())).size;
